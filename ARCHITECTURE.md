...
- Relational market-data scaffold: Liquibase now provisions `market_data_series`, `market_data_candle_segments`, and `market_data_candles` plus dedicated repositories for series filters, coverage-overlap inspection, and range reads; the legacy CSV-backed dataset path still owns execution and telemetry until the later Phase 1 service cutover tasks switch those runtime reads.
