...
- Relational market-data scaffold: Liquibase now provisions `market_data_series`, `market_data_candle_segments`, and `market_data_candles` plus dedicated repositories for series filters, coverage-overlap inspection, and range reads; `MarketDataQueryService` now serves execution and telemetry windows from that relational store when normalized candles exist, while an explicit legacy CSV fallback remains only for still-unmigrated datasets.
