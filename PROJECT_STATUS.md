...
- The first two new Phase 3 strategy implementations are now live in the catalog: `OPENING_RANGE_VWAP_BREAKOUT` ships as a research-only same-day breakout path with shared session-anchor, VWAP, volume, regime, and ATR-stop logic, while `VWAP_PULLBACK_CONTINUATION` adds a research-only higher-timeframe-bias intraday pullback path that waits for session VWAP or EMA support plus RSI reset or fast-EMA reclaim confirmation before re-entry. Both ship with telemetry overlays, paper-safe seed defaults, and frozen pre-code specs in `docs/strategy-specs/OPENING_RANGE_VWAP_BREAKOUT.md` and `docs/strategy-specs/VWAP_PULLBACK_CONTINUATION.md`.
