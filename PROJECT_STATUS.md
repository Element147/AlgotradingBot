...
- The first five new Phase 3 strategy implementations are now live in the catalog: `OPENING_RANGE_VWAP_BREAKOUT` ships as a research-only same-day breakout path with shared session-anchor, VWAP, volume, regime, and ATR-stop logic; `VWAP_PULLBACK_CONTINUATION` adds a research-only higher-timeframe-bias intraday pullback path that waits for session VWAP or EMA support plus RSI reset or fast-EMA reclaim confirmation before re-entry; `EXHAUSTION_REVERSAL_FADE` adds a research-only same-day downside exhaustion fade that only overrides strong downtrends when climactic volume, downside stretch, and bullish reversal confirmation all align; `MULTI_TIMEFRAME_EMA_ADX_PULLBACK` adds a research-only multi-hour continuation path that separates higher-timeframe EMA alignment, pullback definition, and fast-trigger confirmation with ADX or volatility support; and `SQUEEZE_BREAKOUT_REGIME_CONFIRMATION` adds a research-only compression-breakout path that requires squeeze detection, breakout confirmation, momentum alignment, and higher-timeframe regime support before entry. The squeeze path also extends the backtest summary and detail surfaces with strategy-specific breakout failure rate and average hold time metrics so sideways breakout quality can be reviewed directly in the workspace. All five ship with telemetry overlays, paper-safe seed defaults, and frozen pre-code specs in `docs/strategy-specs/OPENING_RANGE_VWAP_BREAKOUT.md`, `docs/strategy-specs/VWAP_PULLBACK_CONTINUATION.md`, `docs/strategy-specs/EXHAUSTION_REVERSAL_FADE.md`, `docs/strategy-specs/MULTI_TIMEFRAME_EMA_ADX_PULLBACK.md`, and `docs/strategy-specs/SQUEEZE_BREAKOUT_REGIME_CONFIRMATION.md`.
