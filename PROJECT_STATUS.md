...
- The first new Phase 3 strategy implementation is now live in the catalog: `OPENING_RANGE_VWAP_BREAKOUT` ships as a research-only same-day breakout path with shared session-anchor, VWAP, volume, regime, and ATR-stop logic, strategy telemetry overlays, paper-safe seed defaults, and a frozen pre-code spec in `docs/strategy-specs/OPENING_RANGE_VWAP_BREAKOUT.md`.
