...
- The representative strategy-audit dataset pack now also lives in `docs/STRATEGY_AUDIT_DATASET_PACK.md`, pairing the verified crypto anchor with a checked-in small-account ETF audit file at `docs/audit-datasets/us-etf-daily-small-account-pack-2024-03-12-to-2026-03-12.csv` (checksum `edbceabab40fecde762158a542dc70678e8c315942e5e572f128e8d2c023fedc`, schema `ohlcv-v1`, symbols `SPY,QQQ,VTI,VT`, `1d`, `US_EQUITIES`) so later reruns are not forced to improvise the equity side.
...
- The valid holdout split now freezes `2024-03-12` to `2025-07-01` as in-sample and `2025-07-01` to `2026-03-12` as out-of-sample. On that split, `SMA_CROSSOVER` stayed positive out of sample at `+7.08%`, `VOLATILITY_MANAGED_DONCHIAN_BREAKOUT`, `DUAL_MOMENTUM_ROTATION`, and `TREND_FIRST_ADAPTIVE_ENSEMBLE` produced no out-of-sample trades in the observed window, `ICHIMOKU_TREND` finished at `-1.49%` on one trade, and the passive benchmark lost `-40.06%`.
- Current catalog actions are now explicit and operator-facing: `BUY_AND_HOLD` stays `baseline only`; `SMA_CROSSOVER` is the sole `paper-monitor candidate`; `DUAL_MOMENTUM_ROTATION`, `VOLATILITY_MANAGED_DONCHIAN_BREAKOUT`, `TREND_FIRST_ADAPTIVE_ENSEMBLE`, and `ICHIMOKU_TREND` stay `research only`; and `TREND_PULLBACK_CONTINUATION`, `REGIME_FILTERED_MEAN_REVERSION`, and `BOLLINGER_BANDS` are `archive candidate` paths until they are materially redesigned.
- A durable nine-strategy scorecard snapshot now lives in `docs/STRATEGY_CATALOG_AUDIT_REPORT.md`, generated from the repo-native `strategyCatalogAudit` runner so the March 27 frozen baseline remains available even after later Phase 3 strategies are added to the live catalog.
...
- The first six new Phase 3 strategy implementations are now live in the catalog: `OPENING_RANGE_VWAP_BREAKOUT` ships as a research-only same-day breakout path with shared session-anchor, VWAP, volume, regime, and ATR-stop logic; `VWAP_PULLBACK_CONTINUATION` adds a research-only higher-timeframe-bias intraday pullback path that waits for session VWAP or EMA support plus RSI reset or fast-EMA reclaim confirmation before re-entry; `EXHAUSTION_REVERSAL_FADE` adds a research-only same-day downside exhaustion fade that only overrides strong downtrends when climactic volume, downside stretch, and bullish reversal confirmation all align; `MULTI_TIMEFRAME_EMA_ADX_PULLBACK` adds a research-only multi-hour continuation path that separates higher-timeframe EMA alignment, pullback definition, and fast-trigger confirmation with ADX or volatility support; `SQUEEZE_BREAKOUT_REGIME_CONFIRMATION` adds a research-only compression-breakout path that requires squeeze detection, breakout confirmation, momentum alignment, and higher-timeframe regime support before entry; and `RELATIVE_STRENGTH_ROTATION_INTRADAY_ENTRY_FILTER` adds a research-only fixed-universe rotation path that ranks only an approved liquid basket, requires explicit absolute-momentum confirmation, and falls back to cash instead of implying a direct short when the basket weakens. The rotation path also adds telemetry overlays for the ranking horizon, absolute filter, and intraday timing trigger so operators can review why the leader was accepted, rejected, or rotated. All six ship with telemetry overlays, paper-safe seed defaults, and frozen pre-code specs in `docs/strategy-specs/OPENING_RANGE_VWAP_BREAKOUT.md`, `docs/strategy-specs/VWAP_PULLBACK_CONTINUATION.md`, `docs/strategy-specs/EXHAUSTION_REVERSAL_FADE.md`, `docs/strategy-specs/MULTI_TIMEFRAME_EMA_ADX_PULLBACK.md`, `docs/strategy-specs/SQUEEZE_BREAKOUT_REGIME_CONFIRMATION.md`, and `docs/strategy-specs/RELATIVE_STRENGTH_ROTATION_INTRADAY_ENTRY_FILTER.md`.
- The shared strategy explainability layer now also covers all six Phase 3 strategies: `frontend/src/features/strategies/strategyProfiles.ts` carries timeframe guidance, stand-aside rules, entry or exit or stand-aside reason labels, indicator evidence checklists, and operator notes, while `frontend/src/components/workspace/ActiveAlgorithmExplainabilityPanel.tsx` plus `frontend/src/features/backtest/BacktestOverviewPanel.tsx` render that same metadata across Backtest, Forward Testing, Paper, and Live so operators can review why a strategy entered, exited, or stood aside from one consistent evidence model.
- The March 27 Phase 3 audit rerun now lives in `docs/PHASE3_STRATEGY_AUDIT_REPORT.md` and keeps the current posture conservative: `OPENING_RANGE_VWAP_BREAKOUT`, `EXHAUSTION_REVERSAL_FADE`, `MULTI_TIMEFRAME_EMA_ADX_PULLBACK`, `SQUEEZE_BREAKOUT_REGIME_CONFIRMATION`, and `RELATIVE_STRENGTH_ROTATION_INTRADAY_ENTRY_FILTER` remain `research-only`, `VWAP_PULLBACK_CONTINUATION` is rejected for active consideration under the current BTC-anchor evidence, and no new strategy is promoted to shadow paper monitoring because the frozen audit pack still lacks an approved intraday ETF anchor for `15m` and `1h` hypotheses.
...
- Forward Testing, Paper, and Live now share one explainability detail surface for active algorithms, so selection consistently exposes entry or exit evidence, signal reason, risk and PnL, position exposure, and recent incidents or overrides without route-specific recomputation of trading logic.
...
- `.\security-scan.ps1 -FailOnFindings`: passed with zero findings.
- `.\run.ps1` backend smoke: passed. Backend health returned `UP`, seeded admin login (`admin` / `dogbert`) succeeded, and the authenticated backtest-algorithm catalog returned the then-current nine-strategy set from local source.
